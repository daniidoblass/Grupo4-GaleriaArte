...
public class EventosConfig implements ActionListener, MouseListener {
		...
		if (source instanceof JButton) {
			...
			if (btn.getText().contains(modelo.getTextoOpcionesConfig()[0])) { // Reestablecer Contraseña
				if (comprobarPassword()) {
					String respuesta = vistaConfigPrincipal.mostrarInputEmergente(modelo.getTextoOpcionesConfig()[0],
							modelo.getTextoDatosConfig()[0]);
				...
				if (comprobarPassword()) {
					String respuesta = vistaConfigPrincipal.mostrarInputEmergente(modelo.getTextoOpcionesConfig()[1],
							modelo.getTextoDatosConfig()[1]);
	...
	private boolean comprobarPassword() {
		...
		try {
			String passwordActual = vistaConfigPrincipal.mostrarInputEmergente("Verificación Usuario",
					"Introduzca su contraseña actual");
			if (!passwordActual.equals("") && !passwordActual.isEmpty()) {
				String usuario = eventos.getUsuario();
				ResultSet rs = conexion.realizarConsultaRS("SELECT * FROM usuarios WHERE nombre='" + usuario
						+ "' AND password = PASSWORD('" + passwordActual + "')");
	...
	private void reestablecerPassword(String respuesta) {
		try {
			if (respuesta.equals("") || respuesta.isEmpty()) {
				...
				String usuario = eventos.getUsuario();
				int num = conexion.realizarUpdateStatement("UPDATE usuarios SET password = PASSWORD('" + respuesta
